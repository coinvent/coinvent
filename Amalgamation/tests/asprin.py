...
def main(prg):
    ...
    enum=0
    #print(prg)
    #roberto: changed prg.getConst for prg.get_const
    maxmodels = prg.get_const("_n") if prg.get_const("_n") != None else 1
    forgetopt  = prg.get_const("_forgetopt")  if prg.get_const("_forgetopt")  != None else 0
    forgetlast = prg.get_const("_forgetlast") if prg.get_const("_forgetlast") != None else 0
    noshowproject = prg.get_const("_noshowproject") if prg.get_const("_noshowproject") != None else 0

    #roberto: another error here and had to replace the following instruction
    #prg.ground("base", [])
    prg.ground([("base", [])])
    while True:
