...
\usepackage{graphicx,url,appendix,multirow}
...
Where steps involve systems such as the interactive theorem prover Isabelle, it is an open question how we script such systems within Coinvent. HETS has some support for proof scripts although the API is not ready for external use.
...
\item Marking relative importance of sentences: by the annotation `\%importance(X)\%`, where X is a number in the range [0,1]. This will be interpreted by the /weaken component. How this is interpreted is tool dependent. It may be interpreted as a probability that the sentence holds.
...
`Concept` type: Concepts can be provided as the source text itself, or as a uri for a file which contains the source text. A DOL file may contain more than one concept. ??This means that here we need an URI for an ontology in a file. We follow the MMT convention to write file_uri?ontology_name.
Ontohub also supports this.
...
For example, it might take in DOL labels (%(label)%) or line-numbers marking sets of inconsistent sentences.
\end{verbatim}
